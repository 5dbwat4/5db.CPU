...
module CSRModule(
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] stvec modified to 0x%016h", csr_val_wb);
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] mtvec modified to 0x%016h", csr_val_wb);
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] sscratch modified to 0x%016h", csr_val_wb);
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] mscratch modified to 0x%016h", csr_val_wb);
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] medeleg modified to 0x%016h", csr_val_wb);
    ...
    always@(posedge clk)begin
        if(rst)begin
            ...
            // $display("[CSR Debug] mideleg modified to 0x%016h", csr_val_wb);
    ...
    assign s_ret=csr_ret[0]&~s_trap&~m_trap&valid_wb;
    assign m_ret=csr_ret[1]&~s_trap&~m_trap&valid_wb;
    ...
    assign s_trap=(s_trap_int|s_trap_exp)&(priv<=2'b01)&valid_wb;
    assign m_trap=(interrupt&valid_wb|except)&~s_trap&valid_wb;
    ...
    assign cosim_interrupt=interrupt & valid_wb;
    assign cosim_cause=except_final.ecause;

    wire [63:0] mstatus_next = is_mstatus_w ? csr_val_wb : mstatus;
    wire [63:0] sstatus_next = is_sstatus_w ? csr_val_wb : sstatus;
    wire [63:0] mie_next     = is_mie_w     ? csr_val_wb : mie;
    wire [63:0] sie_next     = is_sie_w     ? csr_val_wb : sie;
    wire [63:0] mtvec_next   = is_mtvec_w   ? csr_val_wb : mtvec;
    wire [63:0] stvec_next   = is_stvec_w   ? csr_val_wb : stvec;
    wire [63:0] mscratch_next= is_mscratch_w? csr_val_wb : mscratch;
    wire [63:0] sscratch_next= is_sscratch_w? csr_val_wb : sscratch;
    wire [63:0] mepc_next    = is_mepc_w    ? csr_val_wb : mepc;
    wire [63:0] sepc_next    = is_sepc_w    ? csr_val_wb : sepc;
    wire [63:0] mcause_next  = is_mcause_w  ? csr_val_wb : mcause;
    wire [63:0] scause_next  = is_scause_w  ? csr_val_wb : scause;
    wire [63:0] mtval_next   = is_mtval_w   ? csr_val_wb : mtval;
    wire [63:0] stval_next   = is_stval_w   ? csr_val_wb : stval;
    wire [63:0] mip_next     = is_mip_w     ? csr_val_wb : mip;
    wire [63:0] sip_next     = is_sip_w     ? csr_val_wb : sip;
    wire [63:0] medeleg_next = is_medeleg_w ? csr_val_wb : medeleg;
    wire [63:0] mideleg_next = is_mideleg_w ? csr_val_wb : mideleg;

    assign cosim_csr_info.sstatus=sstatus_next;
    assign cosim_csr_info.sie=sie_next;
    assign cosim_csr_info.stvec=stvec_next;
    assign cosim_csr_info.sscratch=sscratch_next;
    assign cosim_csr_info.sepc=sepc_next;
    assign cosim_csr_info.scause=scause_next;
    assign cosim_csr_info.stval=stval_next;
    assign cosim_csr_info.sip=sip_next;
    assign cosim_csr_info.mstatus=mstatus_next;
    assign cosim_csr_info.mie=mie_next;
    assign cosim_csr_info.mtvec=mtvec_next;
    assign cosim_csr_info.mscratch=mscratch_next;
    assign cosim_csr_info.mepc=mepc_next;
    assign cosim_csr_info.mcause=mcause_next;
    assign cosim_csr_info.mtval=mtval_next;
    assign cosim_csr_info.mip=mip_next;
    assign cosim_csr_info.medeleg=medeleg_next;
    assign cosim_csr_info.mideleg=mideleg_next;
